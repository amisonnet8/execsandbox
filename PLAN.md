...
**仕様書§6.4（コード署名）の書き方を調整した（2026-09-09）。**

ユーザーから、生成物への署名についてexecsandbox-buildで対応できないか相談が
あった。調査の結果、次の点が判明・確認できた。

- `go build`自体には署名機能がないが、**darwin/arm64向けだけGoのリンカが
  自動でアドホック署名（身元保証なし）を付与する**。実際にクロスビルドして
  Mach-Oの`LC_CODE_SIGNATURE`を解析し、`codeLimit`が署名データの開始位置
  そのものと一致することを確認した——つまりスタンプ（末尾追記）はこの
  署名の保護範囲の外側で行われるため、既存のアドホック署名を壊さない
  （macOS arm64向け生成物がCIで実際に動いている技術的な裏付け）。
- **本質的な障壁**：ビルダーが生成する任意の中身（利用者のWASM）に開発者の
  証明書で自動署名すると、利用者が作った任意のバイナリに開発者の身元保証が
  付与されてしまう（なりすまし署名に近い）。ビルダー自体（配布物）への署名と、
  生成物への署名は意味論的に別物であり、後者を自動化するのは適切でないと
  判断した。
- 上記を踏まえ、コード実装は行わず、**§6.4の文章表現のみ調整**した
  （英日とも）。「未署名とする」という判断が、あたかもExecSandbox固有の
  例外的な決定であるかのように読める書き方だったのを、「クロスプラット
  フォームCLIツールでは未署名配布が珍しくない」という一般的な文脈の中に
  位置づけ直した。スタンプが署名を壊すという技術的制約は「なお、」で
  始まる補足段落に格下げした。Goのアドホック署名の話は実装依存の細部
  であり、仕様書には持ち込まない（ユーザー判断）。

---

**仕様書§3.1に「通信内容の暗号化（TLS等）を行わない」理由を追記した
（2026-09-09）。**

ユーザーから、サンドボックス間通信のTLS対応要否について相談があった。
AF_UNIXはカーネル内でメモリコピーが完結しネットワークインターフェースへ
パケットとして出ないため、TLSが対応する経路上の盗聴・改ざんという脅威
モデル自体が存在しないこと、アクセス制御は既にソケットファイル・親
ディレクトリのパーミッション（§3.2、`sandbox/listener.go`が待ち受け時に
親ディレクトリを`0o700`で作成済み）で実現できていることを確認し、
`execsandbox`側でのTLSサポートは不要と判断した。この判断理由を仕様書
（英日）§3.1「AF_UNIXを選んだ理由」に一文追記した。コード変更は伴わない。
...
`docs/examples/`・`docs/tour/`（`PLAN.md`・`CLAUDE.md`・`.claude/rules/`は
対象外、日本語のみ継続）を英語版・日本語版のペアにした。命名は英語版が
`.md`、日本語版が`_ja.md`（例：`README.md`／`README_ja.md`）。

- 既存の日本語ファイル26本を`git mv`で`_ja`付きにリネームし（履歴を保持）、
  本文中の相互参照リンクをリネーム後のパスへ更新、ページ冒頭に英語版への
  切り替えリンクを追加した。`docs/spec/execsandbox_spec_ja.md`のみ元から
  この命名だったためリネーム不要。
- 英語版27本を新規作成した。コマンド例・実際の出力（実測値）はそのまま
  維持し、プローズのみ翻訳。最大の作業は`docs/spec/execsandbox_spec.md`
  （934行、§1〜§10全項目の判断理由・検討して退けた案まで含む）。
- 全ページの冒頭にもう一方の言語版への切り替えリンクを1行追加した
  （ユーザー確認済みの方針。README以外にも同様に追加）。
- **翻訳作業中に日本語版の既存の誤りを1件発見し、修正した**:
  `docs/tour/06-the-mailbox-idea_ja.md`が「上限を超えると古い方から捨てられる
  （tail-drop）」と書いていたが、これは誤り（仕様書§3.6・tail-dropの定義
  通り、捨てられるのは新しく届いた方であり、既存のメッセージは捨てない）。
  日本語版・英語版とも正しい記述に統一した。
- `CLAUDE.md`の「言語について」節を更新し、英日併記化の方針（命名規則、
  相互リンクの言語内完結、対象外の明記）を記録した。ついでに「利用者向け
  ドキュメント（`docs/`）」節の「`docs/examples/`・`docs/tour/`は未作成」
  という古い記述も、両方とも作成済みである事実に合わせて更新した。
- **検証**: 日本語ツリー・英語ツリーそれぞれでローカルリンクが解決する
  ことをスクリプトで確認。`make check`（`go vet`・`go test`・`gofmt -l`）
  はドキュメントのみの変更のためgreenのまま。

---

以下は`docs/tour/`新設時点の記録。
