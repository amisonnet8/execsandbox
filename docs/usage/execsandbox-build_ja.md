...
生成される実行ファイルは未署名のまま配布される。クロスプラットフォームで配布
されるCLIツールでは珍しいことではない。macOS・Windowsでは初回起動時に警告が
表示される場合がある（Gatekeeper、SmartScreen）。

なお、末尾への追記が既存の署名を壊すため、スタンプ前に署名しても意味がない
（署名はファイル全体のハッシュで検証される）。配布時に署名が必要な場合は、
生成後に各自で署名し直す。
