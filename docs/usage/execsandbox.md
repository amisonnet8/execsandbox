...
**本ページの内容はフェーズ②完了時点（`-l/--listen` を除く全オプション実装済み）
...
| `-l` | `--listen` | アドレス | 外部接続の待ち受け。1つのみ | **未実装**（フェーズ③で追加予定） |
...
**`-l, --listen` は現時点のビルドには存在しない。** `--help` の出力にも
現れず、指定すると「未定義のオプション」としてパースエラーになる
（下記「起動時オプションの誤りと終了コード」参照）。外部接続はフェーズ③の
スコープであり、実装され次第このページに追記する。
...
**フェーズ③のスコープであり、現時点のビルドにはこのオプション自体が
存在しない。** 指定すると「未定義のオプション」としてパースエラーになる。
以下は仕様書§7.1に基づく将来の書式（実装時に実測値へ差し替える）。
